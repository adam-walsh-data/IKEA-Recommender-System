...
from recommenders.models.SARM.sarm import SARM_trainer
...
def train_SARM(
    ...
    """
    Trains SARM model with provided arguments.
    """
    ...
    val_loss, val_hr, val_ndcg, val_cov, val_r_div, val_r_nov, val_reps = evaluate(
        ...
        model=sarm.network,
        device=sarm.device,
        loss_function=sarm.cross_entropy_loss,
    ...
    for epoch in range(epochs):
        ...
        for n_batch, (s, a, r, s_next, s_len, s_next_len, is_end) in enumerate(
            ...
            sup_loss, q_loss = sarm.train_step(
            ...
            if n_batch + 1 in eval_idx:
                ...
                train_cov = get_coverage(
                    ...
                    num_actions=sarm.network.action_dim,
                    topk=topk_cov,
                )
    ...
    if save_pretrained_embeddings:
        torch.save(
            sarm.network.state_dict()["embedding.weight"],
